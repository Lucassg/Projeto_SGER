...
//                List<Pedido> ListaRota = new ArrayList<>();
            ...
            ListaRota.add(ListaPedidos.get(0));
            QtdePedidoPorRota.add(ListaRota.size());
            ListaRotaFinal.add(ListaRota);
        }
    ...
    public void cria_json_rota(List<Pedido> PedidosRota) throws IOException {
        ...
        for (int k = 0; k < PedidosRota.size(); k++) {
            enderecos.add(PedidosRota.get(k).getCliente().enderecoToString());
        }
        try (BufferedWriter buff_writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("C:\\Users\\Lucas Garcia\\Google Drive\\Projeto Notebook\\Projeto_SGER\\web\\JSON\\enderecos_multiplos_pontos.json", false), "UTF-8"))) {
